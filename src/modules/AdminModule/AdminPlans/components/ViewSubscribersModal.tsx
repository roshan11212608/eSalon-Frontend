...
const SubscriberCard: React.FC<{ item: SubscriberSalon; accentColor: string }> = ({ item, accentColor }) => {
    ...
    <View style={s.card}>
      ...
      <View style={s.cardRight}>
        <Text style={[s.revenue, { color: accentColor }]}>
          {sym}{item.revenueContribution.toLocaleString('en-IN')}<Text style={s.revSub}>/mo</Text>
        </Text>
...
export const ViewSubscribersModal: React.FC<Props> = ({ plan, onClose }) => {
  if (!plan) return null;

  const totalRev = plan.subscribers.reduce((sum, s) => sum + s.revenueContribution, 0);
  const sym = plan.currency === 'INR' ? '₹' : plan.currency;
  const paid = plan.subscribers.filter((s) => s.paymentStatus === 'paid').length;
  const overdue = plan.subscribers.filter((s) => s.paymentStatus === 'overdue').length;

  return (
    <Modal visible={!!plan} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <SafeAreaView style={s.container} edges={['top', 'bottom']}>
        ...
        <View style={[s.summary, { borderLeftColor: plan.color, borderLeftWidth: 4 }]}>
          <View style={s.summaryItem}>
            <Text style={[s.summaryVal, { color: plan.color }]}>{plan.subscribers.length}</Text>
            <Text style={s.summaryLbl}>Total</Text>
          </View>
          <View style={s.summaryDivider} />
          <View style={s.summaryItem}>
            <Text style={[s.summaryVal, { color: '#059669' }]}>{paid}</Text>
            <Text style={s.summaryLbl}>Paid</Text>
          </View>
          <View style={s.summaryDivider} />
          <View style={s.summaryItem}>
            <Text style={[s.summaryVal, { color: '#DC2626' }]}>{overdue}</Text>
            <Text style={s.summaryLbl}>Overdue</Text>
          </View>
          <View style={s.summaryDivider} />
          <View style={s.summaryItem}>
            <Text style={[s.summaryVal, { color: plan.color }]}>
              {sym}{totalRev >= 1000 ? `${(totalRev / 1000).toFixed(1)}K` : totalRev}
            </Text>
